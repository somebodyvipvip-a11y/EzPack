    ...
    def run(self):
        ...
        try:
            ...
            if self.use_upx and upx_bin:
                ...
                # 排除 UPX 已知无法压缩的系统级 DLL，避免产生非致命的内部异常日志
                upx_excludes = [
                    "python*.dll",          # python3.dll / python311.dll 等
                    "VCRUNTIME*.dll",       # VC 运行时
                    "MSVCP*.dll",           # MSVC C++ 标准库
                    "api-ms-win-*.dll",     # Windows API 转发集
                    "*.pyd",                # Python C 扩展模块（UPX 常压缩失败）
                ]
                for excl in upx_excludes:
                    cmd.extend(["--upx-exclude", excl])
                self.log_signal.emit(f"-> 检测到可用 UPX 压缩器，已启用壳压缩策略（已排除易出错系统库）。")
            else:
                cmd.append("--noupx")
